...
class PickAndPlacePerceptionRealController {
public:
  PickAndPlacePerceptionRealController(rclcpp::Node::SharedPtr base_node_) : base_node_(base_node_) {
    RCLCPP_INFO(LOGGER, "Initializing Pick and Place Perception Real Robot Controller...");
    ...
    object_subscription_ = base_node_->create_subscription<custom_msgs::msg::DetectedObjects>(
        "/object_detected", 10,
        std::bind(&PickAndPlacePerceptionRealController::object_callback, this, std::placeholders::_1));

    // Initialize perception variables
    object_detected_ = false;
    execution_started_ = false;

    RCLCPP_INFO(LOGGER, "Pick and Place Perception Real Robot Controller Initialized");
    RCLCPP_INFO(LOGGER, "Waiting for object detection...");
  }

  ~PickAndPlacePerceptionRealController() {
    RCLCPP_INFO(LOGGER, "Pick and Place Perception Real Robot Controller Terminated");
  }
...
int main(int argc, char **argv) {
  ...
  // instantiate class
  PickAndPlacePerceptionRealController pick_and_place_perception_real_node(base_node);

  // keep spinning until execution is complete
  pick_and_place_perception_real_node.spin();
